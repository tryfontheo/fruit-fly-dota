...
def test_lane_placement_temporarily_blocks_tp_including_stale_orders():
    lua=LuaRuntime()
    lua.execute('''
      now=100;orders=0
      GameRules={GetDOTATime=function() return now end}
      local pos=setmetatable({}, {__sub=function() return {Length2D=function() return 5000 end} end})
      h={GetAbsOrigin=function() return pos end,IsMuted=function() return false end,entindex=function() return 1 end}
      tp={IsFullyCastable=function() return true end,entindex=function() return 2 end}
      base={GetAbsOrigin=function() return pos end}
      function ExecuteOrderFromTable(order) orders=orders+1 end
      DOTA_UNIT_ORDER_CAST_POSITION=5
    ''')
