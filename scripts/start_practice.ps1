param([switch]$SelfPlay,[string]$DotaRoot='C:\Program Files (x86)\Steam\steamapps\common\dota 2 beta')
$ErrorActionPreference='Stop'
& "$PSScriptRoot\install_dota.ps1" -DotaRoot $DotaRoot
# Override only this project's installed addon, never stock Dota files.
$(if($SelfPlay){'return "selfplay"'}else{'return true'}) | Set-Content (Join-Path $DotaRoot 'game\dota_addons\fruit_fly_dota\scripts\vscripts\auto_training.lua')
