...
function Activate()
  ...
  Convars:RegisterCommand("fly_play",function()
    ...
    assisted=true;matchMode=true;assist=require("assistance")
    SendToServerConsole("dota_bot_populate")
    print("FLY_ASSISTED: scripted mid route, attack-move, retreat, shopping and skill priority; NOT pure fly play")
  end,"Enable explicitly assisted local bot play",0)
...
function FlyThink()
  ...
  if assisted then assist.shop(h) end
  ...
  req:SetHTTPRequestRawPostBody("application/json",'{"seq":'..requestSeq..',"assisted":'..tostring(assisted)..',"reward":'..deliveredReward..',"obs":['..table.concat(obs,",")..'],"legal":['..table.concat(legal,",")..']}')
  ...
  req:Send(function(response)
    ...
    if assisted and a<=8 then
      order=assist.move(h,a)
